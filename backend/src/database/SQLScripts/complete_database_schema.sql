...
CREATE TABLE employees (
    ...
    status VARCHAR(20),
    full_time_part_time VARCHAR(20),
...
CREATE TABLE deliverables (
    ...
    name VARCHAR(255) NOT NULL,
    description TEXT,
    assigned_employees VARCHAR(255),
    start_date DATE,
    due_date DATE,
...
CREATE TRIGGER audit_users BEFORE INSERT OR UPDATE ON users FOR EACH ROW EXECUTE FUNCTION set_audit_fields();
...
CREATE POLICY "Users can view own profile or admins can view all" ON users
    FOR SELECT USING (
        ...
        user_has_permission(get_current_user_id(), 'users', 'read')
    );

CREATE POLICY "Only admins can create users" ON users
    FOR INSERT WITH CHECK (
        user_has_permission(get_current_user_id(), 'users', 'create')
    );

CREATE POLICY "Users can update own profile or admins can update any" ON users
    FOR UPDATE USING (
        user_id = get_current_user_id() OR
        user_has_permission(get_current_user_id(), 'users', 'update')
    );

CREATE POLICY "Only super admins can delete users" ON users
    FOR DELETE USING (
        user_has_permission(get_current_user_id(), 'users', 'delete')
    );

-- Role permissions policies
CREATE POLICY "Only super admins can manage role permissions" ON role_permissions
    FOR ALL USING (
        get_user_role(get_current_user_id()) = 'super_admin'
    );

-- Clients policies
CREATE POLICY "RBAC clients select" ON clients
    FOR SELECT USING (user_has_permission(get_current_user_id(), 'clients', 'read'));

CREATE POLICY "RBAC clients insert" ON clients
    FOR INSERT WITH CHECK (user_has_permission(get_current_user_id(), 'clients', 'create'));

CREATE POLICY "RBAC clients update" ON clients
    FOR UPDATE USING (user_has_permission(get_current_user_id(), 'clients', 'update'));

CREATE POLICY "RBAC clients delete" ON clients
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'clients', 'delete'));
...
CREATE POLICY "RBAC contracts select" ON contracts
    FOR SELECT USING (user_has_permission(get_current_user_id(), 'contracts', 'read'));

CREATE POLICY "RBAC contracts insert" ON contracts
    FOR INSERT WITH CHECK (user_has_permission(get_current_user_id(), 'contracts', 'create'));

CREATE POLICY "RBAC contracts update" ON contracts
    FOR UPDATE USING (user_has_permission(get_current_user_id(), 'contracts', 'update'));

CREATE POLICY "RBAC contracts delete" ON contracts
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'contracts', 'delete'));

-- Employees policies
CREATE POLICY "RBAC employees select" ON employees
    FOR SELECT USING (
        user_has_permission(get_current_user_id(), 'employees', 'read') OR
        user_id = get_current_user_id()
    );

CREATE POLICY "RBAC employees insert" ON employees
    FOR INSERT WITH CHECK (user_has_permission(get_current_user_id(), 'employees', 'create'));

CREATE POLICY "RBAC employees update" ON employees
    FOR UPDATE USING (
        user_has_permission(get_current_user_id(), 'employees', 'update') OR
        user_id = get_current_user_id()
    );

CREATE POLICY "RBAC employees delete" ON employees
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'employees', 'delete'));

-- Availability policies
CREATE POLICY "RBAC availability select" ON availability
    FOR SELECT USING (
        user_has_permission(get_current_user_id(), 'employees', 'read') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = availability.employee_id AND user_id = get_current_user_id())
    );
...
CREATE POLICY "RBAC client_assignments select" ON client_assignments
    FOR SELECT USING (
        user_has_permission(get_current_user_id(), 'employees', 'read') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = client_assignments.employee_id AND user_id = get_current_user_id())
    );
...
CREATE POLICY "RBAC client_assignments update" ON client_assignments
    FOR UPDATE USING (user_has_permission(get_current_user_id(), 'employees', 'update'));
...
CREATE POLICY "RBAC deliverables select" ON deliverables
    FOR SELECT USING (user_has_permission(get_current_user_id(), 'deliverables', 'read'));

CREATE POLICY "RBAC deliverables insert" ON deliverables
    FOR INSERT WITH CHECK (user_has_permission(get_current_user_id(), 'deliverables', 'create'));

CREATE POLICY "RBAC deliverables update" ON deliverables
    FOR UPDATE USING (user_has_permission(get_current_user_id(), 'deliverables', 'update'));

CREATE POLICY "RBAC deliverables delete" ON deliverables
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'deliverables', 'delete'));

-- Time entries policies
CREATE POLICY "RBAC time_entries select" ON time_entries
    FOR SELECT USING (
        user_has_permission(get_current_user_id(), 'time_entries', 'read') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = time_entries.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC time_entries insert" ON time_entries
    FOR INSERT WITH CHECK (
        user_has_permission(get_current_user_id(), 'time_entries', 'create') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = time_entries.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC time_entries update" ON time_entries
    FOR UPDATE USING (
        user_has_permission(get_current_user_id(), 'time_entries', 'update') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = time_entries.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC time_entries delete" ON time_entries
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'time_entries', 'delete'));

-- Billing policies
CREATE POLICY "RBAC billing select" ON billing
    FOR SELECT USING (user_has_permission(get_current_user_id(), 'billing', 'read'));

CREATE POLICY "RBAC billing insert" ON billing
    FOR INSERT WITH CHECK (user_has_permission(get_current_user_id(), 'billing', 'create'));

CREATE POLICY "RBAC billing update" ON billing
    FOR UPDATE USING (user_has_permission(get_current_user_id(), 'billing', 'update'));

CREATE POLICY "RBAC billing delete" ON billing
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'billing', 'delete'));

-- Expenses policies
CREATE POLICY "RBAC expenses select" ON expenses
    FOR SELECT USING (
        user_has_permission(get_current_user_id(), 'expenses', 'read') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = expenses.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC expenses insert" ON expenses
    FOR INSERT WITH CHECK (
        user_has_permission(get_current_user_id(), 'expenses', 'create') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = expenses.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC expenses update" ON expenses
    FOR UPDATE USING (
        user_has_permission(get_current_user_id(), 'expenses', 'update') OR
        EXISTS(SELECT 1 FROM employees WHERE employee_id = expenses.employee_id AND user_id = get_current_user_id())
    );

CREATE POLICY "RBAC expenses delete" ON expenses
    FOR DELETE USING (user_has_permission(get_current_user_id(), 'expenses', 'delete'));
...
GROUP BY c.client_id, c.client_name, c.primary_contact_name, c.primary_contact_email, c.company_size, c.industry, c.notes, c.created_at, u.full_name;
...
SELECT 
    ...
    u.full_name as created_by_name
FROM expenses ex
LEFT JOIN users u ON ex.created_by = u.user_id
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO users (auth_user_id, email, role, status)
