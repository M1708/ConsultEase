...
async def get_user_profile_by_id(
    ...
    try:
        ...
        return UserProfileResponse(
            ...
            full_name=full_name,
            role=user.role.value if user.role and hasattr(user.role, 'value') else str(user.role) if user.role else 'user',
            status=user.status.value if user.status and hasattr(user.status, 'value') else str(user.status) if user.status else 'active',
            phone=user.phone,
